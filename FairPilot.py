...
if opt_method == 'Grid Search' and checkRF:
    with st.sidebar.expander('**Set HP space:**'):
        ...
        max_samples = np.array(list(map(float, st.multiselect('Max bootstrap samples', ['0.5', '0.6', '0.7', '0.8', '0.0', '1.0'], ['1.0']))))
        ...
        if RF_min_samples_leaf[0] == RF_min_samples_leaf[-1]:
            st.warning("Please select a valid range for min samples lead.")
